...
function validateExpectedTags(validTagIds) {
  ...
  for (const c of allControls) {
    ...
    const enforced = fam === 'IA' || fam === 'AC' || fam === 'SC' || fam === 'AU' || fam === 'CM' || fam === 'SI'
  ...
  if (nonEnforcedArtifactishMissing > 0)
    recordWarning('ExpectedTags', `${nonEnforcedArtifactishMissing} non-IA/AC/SC/AU/CM/SI artifact/mixed objectives have no expectedTags yet (expected during phased rollout; IA, AC, SC, AU, CM, and SI are enforced)`)
...
;(async () => {
  ...
  if (expectedTagsSummary) {
    ...
    console.log(`\nObjective expectedTags: ${e.withTags} mapped, ${e.missingCount} unmapped (non-IA/AC/SC/AU/CM/SI, expected during phased rollout), ${e.waived.length} intentionally waived`)
    console.log(`  enforced families: IA, AC, SC, AU, CM, SI`)
    if (e.waived.length) console.log(`  waived: ${e.waived.join(', ')}`)
    const tags = Object.keys(e.enforcedTagUsage)
    if (tags.length) {
      console.log(`  IA+AC+SC+AU+CM+SI tag usage (${tags.length} distinct tags):`)
