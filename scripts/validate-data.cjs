...
function validateExpectedTags(validTagIds) {
  ...
  for (const c of allControls) {
    ...
    const enforced = fam === 'IA' || fam === 'AC' || fam === 'SC' || fam === 'AU' || fam === 'CM' || fam === 'SI' || fam === 'RA' || fam === 'CA' || fam === 'IR' || fam === 'MP'
  ...
  if (nonEnforcedArtifactishMissing > 0)
    recordWarning('ExpectedTags', `${nonEnforcedArtifactishMissing} non-IA/AC/SC/AU/CM/SI/RA/CA/IR/MP artifact/mixed objectives have no expectedTags yet (expected during phased rollout; IA, AC, SC, AU, CM, SI, RA, CA, IR, and MP are enforced)`)
...
;(async () => {
  ...
  if (expectedTagsSummary) {
    ...
    console.log(`\nObjective expectedTags: ${e.withTags} mapped, ${e.missingCount} unmapped (non-IA/AC/SC/AU/CM/SI/RA/CA/IR/MP, expected during phased rollout), ${e.waived.length} intentionally waived`)
    console.log(`  enforced families: IA, AC, SC, AU, CM, SI, RA, CA, IR, MP`)
    if (e.waived.length) console.log(`  waived: ${e.waived.join(', ')}`)
    const tags = Object.keys(e.enforcedTagUsage)
    if (tags.length) {
      console.log(`  IA+AC+SC+AU+CM+SI+RA+CA+IR+MP tag usage (${tags.length} distinct tags):`)
