...
function validateExpectedTags(validTagIds) {
  ...
  for (const c of allControls) {
    ...
    const enforced = fam === 'IA' || fam === 'AC' || fam === 'SC' || fam === 'AU'
  ...
  if (nonEnforcedArtifactishMissing > 0)
    recordWarning('ExpectedTags', `${nonEnforcedArtifactishMissing} non-IA/AC/SC/AU artifact/mixed objectives have no expectedTags yet (expected during phased rollout; IA, AC, SC, and AU are enforced)`)
...
;(async () => {
  ...
  if (expectedTagsSummary) {
    ...
    console.log(`\nObjective expectedTags: ${e.withTags} mapped, ${e.missingCount} unmapped (non-IA/AC/SC/AU, expected during phased rollout), ${e.waived.length} intentionally waived`)
    console.log(`  enforced families: IA, AC, SC, AU`)
    if (e.waived.length) console.log(`  waived: ${e.waived.join(', ')}`)
    const tags = Object.keys(e.enforcedTagUsage)
    if (tags.length) {
      console.log(`  IA+AC+SC+AU tag usage (${tags.length} distinct tags):`)
